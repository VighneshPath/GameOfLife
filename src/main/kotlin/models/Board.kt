...
class Board(setOfLiveLocations: List<Location>) {
    ...
    fun startGame(iterations: Long): List<Location> {
        for (iteration in 0 until iterations) {
            ...
            currentNeighbourLiveCount.forEach { (location, liveCellCount) ->
                if (willCellSurvive(liveCellCount, location)) {
    ...
    private fun willCellSurvive(liveCellCount: Long, location: Location) =
        liveCellCount == 3L || (liveCellCount == 2L && isCellAlive(location.row, location.column))
